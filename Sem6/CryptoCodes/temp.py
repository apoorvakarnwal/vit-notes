...
def hmac_md5( key,message):
    ...
    ipad= bytes([0x36]*size)
    opad= bytes([0x5C]*size)

    kipad= bytes(a^b for a,b in zip(ipad, key))
    kopad= bytes(a^b for a,b in zip(opad, key))

    hmac=md5(kipad+message)
    hmac=md5(kopad+hmac)
